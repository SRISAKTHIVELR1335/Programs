...
RESPONSIBILITIES
────────────────
✔ Enforce user permissions (vehicle-level and test-level)
✔ Load test metadata from DB (all related tables)
✔ Resolve inputs, execution config, flashing rules, output limits
✔ Load execution callable via diagnostics.loader
✔ Execute asynchronously via diagnostics.runner
✔ Provide UI-facing test listing API
✔ Persist execution results
✔ Support auto-run sessions with VIN tracking, battery voltage, ECU check
✔ Bridge loader auto-run JSON config → runner AutoRunProgramSpec + AutoRunSession

HARD RULES
──────────
❌ No filesystem reads for metadata (DB is authoritative)
❌ No permission logic in loader.py
❌ No DB logic in runner.py

FIXES IN v4.2.0
────────────────
- FIX: ECU status persists FALSE if active check program fails (uses ecu_targets)
- FIX: Stream values persist correctly from runner payloads
- FIX: Enhanced logging for auto-run function resolution
- FIX: Manual VIN input updates DB and in-memory session correctly
- FIX: Robust extraction of ECU status from various payload shapes
"""
    ...
    *,
    ...
    try:
        execute("""
            ...
            ON CONFLICT ON CONSTRAINT uq_stream_session_signal DO UPDATE SET
        ...
        _log_debug(f"Persisted stream value: {signal_name}={signal_value} for session {session_id}")
    except Exception as e:
        _log_warn(f"Failed to upsert stream value [{signal_name}] for session {session_id}: {e}")
    ...
    *,
    ...
    try:
        execute("""
            ...
            ON CONFLICT ON CONSTRAINT uq_ecu_status_session DO UPDATE SET
    ...
    *,
    ...
    for prog_config in sorted(program_configs, key=lambda p: p.get("sort_order", 0)):
        ...
        ctx = ExecutionContext(
            ...
            stream_callback=None,  # Set later
            retry_on_timeout=False,
            retry_on_exception=False,
    ...
    # Precompute limits map for stream persistence
    limits_by_program: Dict[str, Dict[str, Dict[str, Any]]] = {}
    for s in specs:
        lims: Dict[str, Dict[str, Any]] = {}
        for lim in (s.output_limits or []):
            sig = lim.get("signal")
            if sig:
                lims[sig] = lim
        limits_by_program[s.program_id] = lims
    ...
    def on_stream_data_wrapper(sid: str, pid: str, stream_payload: Any):
        ...
        """
        _log_debug(f"Stream data received: session={sid}, program={pid}")
        ...
        def handle_data_dict(data: Dict[str, Any]):
            for signal_name, raw_value in data.items():
                ...
                if num_value is None:
                    # numeric column; skip non-numeric signals
                    continue
    ...
    # CRITICAL: Set stream callbacks on ExecutionContext BEFORE passing to runner
    ...
    for spec in specs:
        if spec.program_type == "stream":
            ...
            _log_info(f"  Set stream_callback for {spec.program_id}")
...
def init_service():
    _log_info("Diagnostic service v4.1.1 initialized")
