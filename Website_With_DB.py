...
──────────────────────────────────────────────────────────
1) VIN saved to app.auto_run_sessions must display in UI:
   - tests_root now accepts ?auto_run_session_id=...
   - server queries app.auto_run_sessions.vin and passes current_vin to tests.html

2) Auto-run failure must NOT navigate forward:
   - This is enforced in tests.html (client-side fail-closed)
   - This file ensures /api/run_auto_programs always returns ok/error consistently

3) Scan preview:
   - tests.html prefers html5-qrcode preview (browser camera) on desktop+mobile
   - Backend OpenCV scan still exists as fallback
   - This file adds OPTIONAL backend preview endpoint:
       GET /api/scan/<scan_id>/frame

4) Icons:
   - Icons must be stored under /static and served via url_for('static', filename=...)
   - Service layer must return icon fields for ECUs/health tabs (service.py fix)
"""
...
def tests_root():
    ...
    # NEW: VIN display support (passed from auto-run UI)
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("NIRIX Diagnostics v3.0.5")
