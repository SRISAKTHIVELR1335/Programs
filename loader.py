...
try:
    from database import query_one, query_all, execute  # type: ignore
except ImportError:
...
def _sync_ecus_to_db(vehicle_id: int, data: Dict, stats: Dict):
    ...
    for ecu in ecus_list:
        ...
        # Upsert global diagnostic folder
        ...
        # Upsert vehicle-specific ECU entry
...
def _upsert_test_to_db(
    ...
    # Sync child tables (delete-and-reinsert).
    # NOTE: These should ideally run within a single transaction to avoid
    # orphaned deletes if a subsequent insert fails. The caller or the
    # database module is expected to manage the transaction boundary.
