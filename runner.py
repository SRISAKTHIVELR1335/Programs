...
RESPONSIBILITIES
────────────────
✔ Execute diagnostic test functions asynchronously
✔ Honor execution config (timeout, retries, mode)
✔ Support all execution modes: single, live, stream, flashing
✔ Retry on retryable UDS NRCs
✔ Timeout enforcement (cooperative cancel + best-effort stop)
✔ Cancel / Pause / Resume (cooperative via TaskContext)
✔ Live per-task logs (in-memory, bounded)
✔ Progress callbacks (UI-safe)
✔ Output validation (LSL/USL) with multi-signal reporting
✔ Persist results via injected callback
✔ Smart kwargs injection based on function signature
✔ Support Run All functionality (sequential/parallel)
✔ Auto-Run sessions with VIN tracking + callbacks
✔ Generator-based streaming support with per-signal limit coloring

RULES
─────
❌ No database access (DB-agnostic)
❌ No filesystem access
❌ No schema validation

CONTRACTS
─────────
- service.py must enforce permissions BEFORE calling execute_test_async
- loader.py / service.py must normalize execution_mode (single/live/stream/flashing)
- Tests return dict[str, numeric] for output limit validation (single-shot)
- Streaming tests can be generator functions that yield dicts
- Long-running tests MUST call context.checkpoint() periodically
- Auto-run results are forwarded via callbacks (no DB in runner)

FIXES IN v3.5.0
────────────────
- FIX-1:  Case-insensitive value extraction for VIN/VOLTAGE/etc
- FIX-2:  Stream callback invoked for non-generator single-shot functions
- FIX-3:  ECU status extraction with multiple key name variants
- FIX-4:  Stream data normalization handles unwrapped dicts
- FIX-5:  Better single-key dict value extraction
- FIX-6:  Auto-run session closes correctly once all singles done + streams started
- FIX-7:  Skipped programs marked as FAILED (not silently passed)
- FIX-8:  VIN manual-input modal triggered reliably when VIN program fails/skipped
- FIX-9:  Stream programs marked "running" immediately so UI sees them active
...
- FIX-12: Auto-run result value extraction improved for nested/single-key dicts
- FIX-13: Stream ExecutionContext timeout ALWAYS set to 0 (infinite) — prevents
          premature timeout killing generator before first yield
- FIX-14: Runner preserves service's stream_callback on ExecutionContext instead of
          overwriting it; chains both service (DB-persist) and runner (UI forward)
          callbacks so auto_run_stream_values rows are actually written
"""
...
class ExecutionContext:
    ...
    def __post_init__(self):
        ...
        # ── FIX-13: Streams ALWAYS get infinite timeout ──────────────────
        # Streams run indefinitely; their lifecycle is controlled by
        # cancellation (stop_auto_run_session / cancel_task), not timeout.
        # Any non-zero timeout_sec from JSON config would cause
        # _run_with_retries to kill the stream thread prematurely via
        # exec_thread.join(timeout) before the generator ever yields.
        #
        # BEFORE (broken — only overrode when timeout == DEFAULT_TIMEOUT_SEC):
        #   if self.execution_mode == "stream" and self.timeout_sec == DEFAULT_TIMEOUT_SEC:
        #       self.timeout_sec = STREAM_NO_TIMEOUT
        #
        # AFTER (fixed — streams always get infinite timeout):
    ...
    *,
    ...
    try:
        with contextlib.redirect_stdout(stdout_buf), contextlib.redirect_stderr(stderr_buf):
            ...
            # FIX-2: Call stream callback for single-shot functions that have one set
    ...
    try:
        with contextlib.redirect_stdout(stdout_buf), contextlib.redirect_stderr(stderr_buf):
            ...
            for yielded in gen:
                ...
                # FIX-4: Handle non-dict yields gracefully
                ...
                # FIX-4: If 'data' key is missing, treat whole dict as data
...
def _run_with_retries(
    *,
    ...
    while attempts <= ctx.max_retries:
        ...
        # FIX-4 / FIX-10: Choose execution function correctly
...
def _extract_ecu_statuses(output: Any) -> List[Dict[str, Any]]:
    ...
    if isinstance(output, dict):
        # FIX-3: Case-insensitive key lookup
        ...
        # FIX-3: Handle details dict format {"BMS": true/false}
...
def _extract_result_value(output: Any) -> Optional[str]:
    ...
    # FIX-1: Case-insensitive key lookup
    output_lower = {k.lower(): v for k, v in output.items()}
    ...
    # FIX-5 / FIX-12: Single-key dict → use its value directly
...
class AutoRunSession:
    ...
    # FIX-9: Track which streams have been started
        ...
        try:
            ...
            for prog in single_programs:
                ...
                try:
                    ...
                    # FIX-1 / FIX-5 / FIX-12: Enhanced value extraction
            ...
            # ── FIX-10: Start stream programs regardless of single failures ──
            ...
            for prog in stream_programs:
                ...
                #
                # service.start_auto_run already sets prog.ctx.stream_callback
                # to on_stream_data_wrapper which persists values to DB via
                # _persist_stream_value.  We must NOT overwrite it — instead,
                # chain both callbacks so DB persistence AND UI forwarding
                # both happen.
                #
                # BEFORE (broken — overwrote service callback):
                #   prog.ctx.stream_callback = make_stream_cb(prog.program_id)
                #
                # AFTER (fixed — chains existing + new):
                ...
                def make_stream_cb(p_id: str, existing: Optional[Callable] = None):
                    ...
                    def stream_cb(tid: str, data: Dict[str, Any]):
                        # 1. Call existing callback first (service's
                        #    DB-persisting on_stream_data_wrapper)
                        ...
                        #    (passed by caller, typically for UI updates)
                ...
                try:
                    ...
                    # FIX-9: Mark stream as "running" immediately (not "pending")
                    ...
                    # FIX-7: Mark failed streams explicitly
            ...
            # FIX-6 / FIX-9: Mark streams as started so session completion logic works
...
def submit_manual_vin(session_id: str, program_id: str, vin_value: str) -> Dict[str, Any]:
    ...
    if existing:
        ...
        existing.error_message = None  # FIX-8: Clear previous error
    else:
...
def cleanup_auto_run_sessions(max_age_sec: float = 3600) -> int:
    ...
    with _AUTO_RUN_LOCK:
        ...
        for sid, session in _AUTO_RUN_SESSIONS.items():
            # Check if all tasks are terminal and session ended
            all_terminal = all(
...
def _init_runner():
    ...
    _log_info(f"Runner v3.5.0 max_tasks={MAX_CONCURRENT_TASKS}")
